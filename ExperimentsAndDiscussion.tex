...
All images were obtained with an automatic procedure that systematically sampled images from a view sphere, resulting in around $38.0\times10^3$ training and $10.0\times10^3$ test images. The training images depict objects in isolation with a black background, while the test images originate from twenty table-top scenes with arbitrarily arranged objects. The training images of each object are captured from a systematically sampled view sphere with $10^{\circ}$ step in elevation (from $85^\circ$ to $-85^\circ$) and $5^\circ$ step in azimuth. In this work we only consider the cropped images of objects in testing scenes, resulting a testing set with size $69.5\times10^3$, while the scene context information can be exploited with conditional random field in the final experiment.
...
\begin{figure}[H]
		\centering
		\includegraphics[height=9cm, width=15cm]{tless_domain_diff}
		\caption{Example of masked images of objects from 28 categories which are not belonging to WRGBD categories, which are treated as OOD data samples.}		
